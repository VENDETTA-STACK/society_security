...
router.post("/addMember",async function(req,res,next){
    try {
                ...
                $match: {
                    $and: [
                        ...
                        { 
                            society: { 
                                $elemMatch: { 
                                    societyId: mongoose.Types.ObjectId(checkSociety[0]._id) 
                                } 
                            } 
                        },
                        // { society: { $elemMatch: { wingId: mongoose.Types.ObjectId(wingId) } } }, Told By Anirudh to Remove 
                        // { society: { $elemMatch: { flatId: mongoose.Types.ObjectId(flatId) } } }, Confirm By Ashish bhai
                    ]
                }
            }
        ]);
        ...
        let societyAdminList = await getSocietyAdmin(societyId,wingId);
...
router.post("/updateMemberPlayerId",async function(req,res,next){
    try {
        const { MobileNo , playerId , DeviceType , memberId , isAdmin } = req.body;
...
//Add Member Resources(Staff & Watchman)-----------------MONIL----------------03/06/2021
router.post("/addStaff_v1", staffImg.single("staffImage") ,async function(req,res,next){
    try {
        ...
        if(staffCategory == "Watchmen"){
            ...
            if(addStaffIs != null){
                try {
                    await addStaffIs.save();    
                } catch (error) {
                    return res.status(500).json({ IsSuccess: false , Message: error.message });
                }

                if(wingId != undefined && wingId != null && wingId != ""){
                    for(let i=0;i<wingsAre.length;i++){
                        let wingIdIs = String(wingsAre[i]);
    
                        let update = {
                            $push: {
                                workingLocation: { wingId: mongoose.Types.ObjectId(wingIdIs) }
                            }
                        }
                        let updateStaff = await staffSchema.updateOne({_id: addStaffIs._id},update);
                    }
                }
...
//Update Staff Data-----------MONIL-------------03/06/2021
router.post("/updateStaffDetails",async function(req,res,next){
    try {
        const { isWatchman , staffId , Name , VehicleNo , ContactNo1 , Address , staffImage } = req.body;
        ...
        if(isWatchman == true){
            ...
            if(getWatchman.length == 1){
                let imagePath;
                if(staffImage != undefined && staffImage != null && staffImage != ""){
                    const path = 'uploads/watchman/'+ 'watchman_' +Date.now()+'.png'
                        
                    const base64Data = staffImage.replace(/^data:([A-Za-z-+/]+);base64,/, '');
                ...
                let update = {
                    Name: Name != undefined ? Name : getWatchman[0].Name,
                    watchmanImage: imagePath != undefined ? imagePath : getWatchman[0].watchmanImage,
                    VehicleNo: VehicleNo != undefined ? VehicleNo : getWatchman[0].VehicleNo,
                    Address: Address != undefined ? Address : getWatchman[0].Address,
                }

                let updateWatchman = await watchmanSchema.findByIdAndUpdate(staffId,update);

                return res.status(200).json({ IsSuccess: true , Data: 1 , Message: "Watchman Details Updated" });
            }else{
            ...
            if(getStaff.length == 1){
                let imagePath;
                if(staffImage != undefined && staffImage != null && staffImage != ""){
                    const path = 'uploads/staff/'+ 'staff_' +Date.now()+'.png'
                ...
                let update = {
                    Name: Name != undefined ? Name : getStaff[0].Name,
                    staffImage: imagePath != undefined ? imagePath : getStaff[0].staffImage,
                    ContactNo1: ContactNo1 != undefined ? ContactNo1 : getStaff[0].ContactNo1,
                    VehicleNo: VehicleNo != undefined ? VehicleNo : getStaff[0].VehicleNo,
                    Address: Address != undefined ? Address : getStaff[0].Address,
                }

                let updateStaff = await staffSchema.findByIdAndUpdate(staffId,update);

                return res.status(200).json({ IsSuccess: true , Data: 1 , Message: "Staff Details Updated" });
            }else{
                return res.status(200).json({ IsSuccess: true , Data: 0 , Message: "No Staff Found" });
            }
        }
    } catch (error) {
        res.status(500).json({ IsSuccess: false , Message: error.message });
...
//Delete Invite Guest----------------MONIL-------07/06/2021
router.post("/deleteInviteGuest",async function(req,res,next){
    try {
        const { inviteGuestId } = req.body;
        ...
        let getGuest = await inviteGuestSchema.aggregate([
            {
                $match: {
                    _id: mongoose.Types.ObjectId(inviteGuestId)
                }
            }
        ]);

        if(getGuest.length == 1){
            let deleteGuest = await inviteGuestSchema.findByIdAndDelete(inviteGuestId);

            return res.status(200).json({ IsSuccess: true , Data: 1 , Message: "Invitation Deleted" });
        }else{
            return res.status(200).json({ IsSuccess: true , Data: 0 , Message: "No Invitation Found" });
        }
    } catch (error) {
        res.status(500).json({ IsSuccess: false , Message: error.message });
    }
        ...
        if(sendBy == 0){
            ...
            if(senderIs.length > 0){
                ...
                if(receiverMemberIds.length > 0){
                    memberTokens = await getMemberPlayerId_mute_unmute(receiverMemberIds,senderId);
                }
            ...
            if(senderIs.length == 1){
                ...
                if(receiverMemberIds.length > 0){
                    memberTokens = await getMemberPlayerId_mute_unmute(receiverMemberIds);
                }
...
//rejectBy - true - Reject By Sender
//rejectBy - false - Reject By Receiver
router.post("/rejectCall",async function(req,res,next){
    try {
        ...
        if(checkCall.length == 1){
            ...
            let getEntry = await unknownVisitorSchema.aggregate([
                {
                    $match: {
                        _id: mongoose.Types.ObjectId(callingId)
                    }
            ...
            if(getEntry.length == 0){
                return res.status(200).json({ IsSuccess: true , Data: [] , Message: `No Entry Found` });
            }
    
            // let flatId = getEntry[0].flatId;
            let watchmanId = getEntry[0].watchmanId;
            let memberId = getEntry[0].responseMemberId;

            // let memberIds = await getMemberOfFlat([flatId]);
            ...
            if(rejectBy == true){
                ...
                let updateMemberCall = await memberSchema.findByIdAndUpdate(memberId,update);
                let updateWatchmanCall = await watchmanSchema.findByIdAndUpdate(watchmanId,update);

                let receiverWatchmanTokens = await getSingleWatchmanPlayerId(watchmanId);

                if(receiverWatchmanTokens.length > 0){
                    let titleIs = `Incoming Call Is Rejected By Sender(Member)`;
                    let bodyIs = `Sender Reject the Call`;

                    let notiDataIs = {
                        NotificationType: "RejectVideoCallingBySender",
                        CallResponse: 2,
                        CallResponseIs: "Rejected",
                        CallingId: callingId,
                        content_available: true, 
                    ...
                    receiverWatchmanTokens.forEach(tokenIs=>{
                        if(tokenIs.DeviceType == "IOS"){                                    
                            let message = { 
                                app_id: process.env.APP_ID_IOS,
                                contents: {
                            ...
                            sendOneSignalNotification(message,false,true,tokenIs.watchmanId,memberId,"RejectVideoCallingBySender","IOS");
                        }else{
                            let message = { 
                                app_id: process.env.APP_ID_W,
                                contents: {
                            ...
                            sendOneSignalNotification(message,false,true,tokenIs.watchmanId,memberId,"RejectVideoCallingBySender","Android");
                        }
                    });
                }
                ...
                return res.status(200).json({ 
                    IsSuccess: true, 
                    Data: 1,
                    Message: "Call Rejected By Sender"
                });
            }else if(rejectBy == false){
                ...
                let senderMemberToken = await getSingleMemberPlayerId(memberId);

                if(senderMemberToken.length > 0){
                    let titleIs = `Your Call Is Rejected By Receiver(Watchman)`;
                    let bodyIs = `Receiver Is Busy`;

                    let notiDataIs = {
                        NotificationType: "RejectVideoCallingByReceiver",
                        CallResponse: 2,
                        CallResponseIs: "Rejected",
                        CallingId: callingId,
                        content_available: true, 
                    ...
                    senderMemberToken.forEach(tokenIs=>{
                        if(tokenIs.DeviceType == "IOS"){                                    
                            let message = { 
                                app_id: process.env.APP_ID_IOS,
                                contents: {
                                ...
                                content_available: 1,
                                include_player_ids: [tokenIs.playerId],
                                // ios_sound: "ac902738-f0b1-495e-8fe3-be54786d8f0c"
                            };
                            sendOneSignalNotification(message,true,false,tokenIs.memberId,watchmanId,"RejectVideoCallingByReceiver","IOS");
                        }else{
                            let message = { 
                                app_id: process.env.APP_ID,
                                contents: {
                            ...
                            sendOneSignalNotification(message,true,false,tokenIs.memberId,watchmanId,"RejectVideoCallingByReceiver","Android");
                        }
                    });
                }
                ...
                return res.status(200).json({ 
                    IsSuccess: true, 
                    Data: [], 
                    Message: "Please Pass True / False for rejectBy Field" 
                });
            }
        }
...
//rejectBy - true - Reject By Sender
//rejectBy - false - Reject By Receiver
router.post("/rejectCall_v1",async function(req,res,next){
    try {
        const { callingId , rejectBy } = req.body;
...
//Reject Call for member or watchman or for both----------------MONIL--------10/06/2021
router.post("/rejectCallForMemberWatchman",async function(req,res,next){
    try {
        const { memberId , watchmanId } = req.body;
        ...
        if(memberId != undefined && memberId != null && memberId != "" && 
            watchmanId != undefined && watchmanId != null && watchmanId != ""){

                let updateMemberCall = await memberSchema.findByIdAndUpdate(memberId,update);
                let updateWatchmanCall = await watchmanSchema.findByIdAndUpdate(watchmanId,update); 

                return res.status(200).json({ IsSuccess: true , Data: [] , Message: "Call Rejected" });

        }else if(memberId != undefined && memberId != null && memberId != "" && 
                (watchmanId == undefined || watchmanId == null || watchmanId == "")){
        ...
        }else if(watchmanId != undefined && watchmanId != null && watchmanId != "" &&
...
//Get Society Staff--------------------MONIL--------------------03/06/2021
router.post("/getSocietyStaff_v1",async function(req,res,next){
    try {
        const { societyId } = req.body;
...
router.post("/answerOfPollQuestion_v1",async function(req,res,next){
    try {
...
router.post("/answerOfPollQuestion",async function(req,res,next){
    try {
        const { pollQuestionId , pollOptionId , memberId } = req.body;
...
//Get Only Full fill Flats----------------------MONIL-----------17/04/2021
router.post('/getOccupiedFlats_v1',async function(req,res,next){
    try {
        const { societyId , wingId } = req.body;
        ...
        let getFlats;
        if(wingId != null && wingId != undefined && wingId != ""){
            let wingList = wingId.split(",");

            getFlats = [];

            if(wingList.length > 0){
                for(let i=0;i<wingList.length;i++){
                    let wingIs = wingList[i];
                    let flats = await flatSchema.aggregate([
                        {
                            $match: {
                                $and: [
                                    { societyId: mongoose.Types.ObjectId(societyId) },
                                    { wingId: mongoose.Types.ObjectId(wingIs) },
                                    { parentMember: { $exists: true } }
                                ]
                            }
                        },
        ...
        if(getFlats.length > 0){
            return res.status(200).json({ 
                IsSuccess: true, 
                Count: getFlats.length, 
                Data: getFlats, 
                Message: "Flat Data Found" 
            });
        }else{
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Flat Data Found" });
        }
...
//Get Only Full fill Flats----------------------MONIL-----------12/06/2021
router.post('/getOccupiedFlats_v2',async function(req,res,next){
    try {
        const { societyId , wingId } = req.body;
...
router.post("/responseToUnknownVisitorEntry",async function(req,res,next){
    try {
        const { entryId , memberId , societyId , response , flatId } = req.body;
...
router.post("/logout",async function(req,res,next){
    try {
        ...
        if(getMemberToken.length == 1){
            let tokenId = getMemberToken[0]._id;
            let deleteMemberToken = await memberTokenSchema.findByIdAndDelete(tokenId);
        }
...
async function getSocietyAdmin(societyId,wingId){
