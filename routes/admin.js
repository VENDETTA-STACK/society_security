...
const visitorSubCategorySchema = require('../models/visitorSubCategory');
...
router.post("/getMemberOccupiedWing",async function(req,res,next){
    try {
        ...
        let getWings = await flatSchema.aggregate([
            {
                $match: {
                    $and: [
                        { societyId: mongoose.Types.ObjectId(societyId) },
                        { memberIds: { $not: { $size: 0 } } }
                    ]
                }
            }
        ]);
...
//Get All Flats of society----------------MONIL----------------------12/06/2021
router.post("/getFlatsOfSociety_v1",async function(req,res,next){
    try {
        const { societyId , wingId } = req.body;
...
//Get Flat Of Society using societyCode and wingId ------------MONIL-----10/04/2021
router.post("/getSocietyFlats_v1",async function(req,res,next){
    try {
        const { wingId } = req.body;
        let authToken = req.headers['authorization'];
...
//Update Society Rules----------MONIL----------07/06/2021
router.post("/updateSocietyRules_v1", societyDocsFiles.single("FileAttachment") ,async function(req,res,next){
    try {
        const { adminId , wingId , societyId , ruleId , Title , Description } = req.body;
        ...
        if(checkAdmin.length == 0){
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: "Only Admin Can Edit The Rules" });
        }
        ...
        if(getSocietyRule.length == 1){
            ...
            res.status(200).json({ IsSuccess: true , Data: 0 , Message: "Only WingAdmin Can Edit The Rules" });
        }
    } catch (error) {
        res.status(500).json({ IsSuccess: false , Message: error.message });
    }
...
//Get List of unapproved Member List------------MONIL---------02/04/2021
router.post("/getMemberApprovalList_v1",async function(req,res,next){
    try {
        const { societyId , wingId } = req.body;
        ...
        if(getMembers.length > 0){
            return res.status(200).json({ 
                ...
                Data: getMembers, 
                Message: "Pending for Approval Member List Found" 
            });
        }else{
            return res.status(200).json({ IsSuccess: true , Data: getMembers , Message: "No members found for Pending Approval" });
        }
...
router.post("/memberApproval",async function(req,res,next){
    try {
        const { memberId , isVerify , societyId , adminId } = req.body;
        ...
        if(isVerify == true){
            ...
            let titleIs = `You are verified for society ${checkSociety[0].Name}`; 
            ...
            let titleIs = `Your Verification for society ${checkSociety[0].Name}`; 
...
//Approve / DisApprove the member------------------MONIL--------02/04/2021
router.post("/memberApproval_v1",async function(req,res,next){
    try {
...
router.post("/addPollingQuestion",async function(req,res,next){
    try {
        const { pollQuestion , societyId , pollOption , adminId } = req.body;
        ...
        let addPollQuestion = await new pollingQuestionSchema({
                ...
                memberTokens.forEach(tokenIs=>{
                    if(tokenIs.muteNotificationAudio == false){
                        if(tokenIs.DeviceType == "IOS"){
                            ...
                            sendOneSignalNotification(message,true,true,tokenIs.memberId,adminId,"AddPoll","IOS");
                        }else{
                            let message = { 
                                app_id: process.env.APP_ID,
                                contents: {
                            ...
                            sendOneSignalNotification(message,true,true,tokenIs.memberId,adminId,"AddPoll","Android");
                        }
                    }
...
//Add Polling Question------------------------MONIL----------------------24/03/2021
router.post("/addPollingQuestion_v1",async function(req,res,next){
    try {
        ...
        if(addPollQuestion != null){
            ...
            if(addPollQuestion.pollFor.isForWholeSociety == false){
                memberIdList = await getSocietyWingMembers(societyId,wingIds);
            }else{
                memberIdList = await getSocietyMember(societyId);
            }
...
//Add Polling Question------------------------MONIL----------------------24/03/2021
router.post("/addPollingQuestion_v2", societyDocsFiles.single("FileAttachment") ,async function(req,res,next){
    try {
        const { pollQuestion , societyId , pollOption , adminId , wingIds } = req.body;
        ...
        if(checkAdmin.length == 0){
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: "Only society admins can add Poll" });
        }
        ...
        if(wingIds != undefined && wingIds != null && wingIds != ""){
            ...
            let societyIs = String(societyId);
            let wingList = await getSocietyWings(societyIs);
            addPollQuestion = await new pollingQuestionSchema({
                pollQuestion: pollQuestion,
                societyId: societyId,
                pollFor: {
        ...
        if(addPollQuestion != null){
            ...
            let pollOptionList = pollOption.split(",");
            let checkExistOption = await pollingOptionsSchema.aggregate([
                {
                    $match: {
                        $and: [
                            { pollQuestionId: mongoose.Types.ObjectId(pollQuestionIdIs) },
                            { pollOption: { $in: pollOptionList } }
                        ]
                    }
                }
            ]);
            ...
            if(checkExistOption.length == 1){
                return res.status(200).json({ IsSuccess: true , Data: [] , Message: `Polling option ${pollOptionList} already exist` });
            }
    
            pollOptionList.forEach(async function(option){
                let addPollingOptions = await new pollingOptionsSchema({
                    pollQuestionId: pollQuestionIdIs,
                    pollOption: option
                });
...
router.post("/getAllPollingQuestion_v1",async function(req,res,next){
    try {
        const { societyId , memberId , wingId } = req.body;
...
//Edit Poll Question--------------------------MONIL---------------------07/06/2021
router.post("/updatePollQuestion",async function(req,res,next){
    try {
        const { pollQuesId , pollQuestion , pollOption } = req.body;
        ...
        let getPollQues = await pollingQuestionSchema.aggregate([
            {
                $match: {
                    _id: mongoose.Types.ObjectId(pollQuesId)
                }
            }
        ]);

        if(getPollQues.length == 1){
            let update = {
                pollQuestion: pollQuestion
            }

            let updateQuest = await pollingQuestionSchema.findByIdAndUpdate(pollQuesId,update);

            if(pollOption != undefined && pollOption != null && pollOption != []){
                if(pollOption.length > 0){
                    let deletePollOption = await pollingOptionsSchema.deleteMany({ pollQuestionId: mongoose.Types.ObjectId(pollQuesId) });
                    pollOption.forEach(async option=>{
                        let addPollQues = await new pollingOptionsSchema({
                            pollQuestionId: pollQuesId,
                            pollOption: option
                        });

                        if(addPollQues != null){
                            await addPollQues.save();
                        }
                    });
                }
            }

            return res.status(200).json({ IsSuccess: true, Data: 1 , Message: "Poll Updated" });
        }else{
            return res.status(200).json({ IsSuccess: true , Data: 0 , Message: "No Poll Found" });
        }
    } catch (error) {
...
//Edit Poll Question--------------------------MONIL---------------------07/06/2021
router.post("/updatePollQuestion_v1",societyDocsFiles.single("FileAttachment"),async function(req,res,next){
    try {
        const { pollQuesId , pollQuestion , pollOption } = req.body;
        ...
        if(getPollQues.length == 1){
            ...
            if(pollOption != undefined && pollOption != null && pollOption != ""){
                let pollOptionList = pollOption.split(",")
                let deletePollOption = await pollingOptionsSchema.deleteMany({ pollQuestionId: mongoose.Types.ObjectId(pollQuesId) });
                pollOptionList.forEach(async option=>{
                    let addPollQues = await new pollingOptionsSchema({
                        pollQuestionId: pollQuesId,
                        pollOption: option
                    });
                ...
                let deletePollAnswer = await pollingAnswersSchema.deleteMany({ pollQuestionId: pollQuesId });
            }
...
//Get Dashboard Counter-------------MONIL-------03/04/2021
router.post("/getDashboardCount_v1",async function(req,res,next){
    try {
        const { societyId , wingId } = req.body;
        ...
        if(societyId != undefined && societyId != null && societyId != "" && wingId != undefined && wingId != null && wingId != ""){
            ...
            let getDocCount = await societyDocsSchema.aggregate([
                {
                    $match: {
                        $and: [
                            { societyId: mongoose.Types.ObjectId(societyId) },
                            { "docFor.isForWholeSociety": false },
                            { "docFor.wingId": mongoose.Types.ObjectId(wingId) }
                        ]
                    }
                }
            ]);
            ...
            let getRulesCount = await societyRulesSchema.aggregate([
                {
                    $match: {
                        $and: [
                            { societyId: mongoose.Types.ObjectId(societyId) },
                            // { "ruleFor.isForWholeSociety": false },
                            // { "ruleFor.wingId": mongoose.Types.ObjectId(wingId) }
                        ]
                    }
                }
            ]);
            ...
            let getNoticeBoardCount = await noticeBoardSchema.aggregate([
                {
                    $match: {
                        $and: [
                            { societyId: mongoose.Types.ObjectId(societyId) },
                            { "noticeFor.isForWholeSociety": false },
                            { "noticeFor.wingId": mongoose.Types.ObjectId(wingId) }
                        ]
                    }
                }
            ]);
            ...
            let getGalleryCount = await gallerySchema.aggregate([
                {
                    $match: {
                        $and: [
                            { societyId: mongoose.Types.ObjectId(societyId) },
                            { "galleryFor.isForWholeSociety": false },
                            { "galleryFor.wingId": mongoose.Types.ObjectId(wingId) }
                        ]
                    }
                }
            ]);
            ...
            let getPollingCount = await pollingQuestionSchema.aggregate([
                {
                    $match: {
                        $and: [
                            { societyId: mongoose.Types.ObjectId(societyId) },
                            { "pollFor.isForWholeSociety": false },
                            { "pollFor.wingId": mongoose.Types.ObjectId(wingId) }
                        ]
                    }
                }
            ]);
            ...
            if(DashboardCount != null){
                return res.status(200).json({ IsSuccess: true , Data: DashboardCount , Message: "Dashboard Counts Found For Wing" });
            }else{
                return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Dashboard Counts Found For Wing" });
            }
        }else if(societyId != undefined && societyId != null && societyId != "" && (wingId == undefined || wingId == null || wingId == "")){
            let getMemberCount = await memberSchema.aggregate([
                {
                    $match: {
                        society: { $elemMatch: { societyId: mongoose.Types.ObjectId(societyId) } }
            ...
            if(DashboardCount != null){
                return res.status(200).json({ IsSuccess: true , Data: DashboardCount , Message: "Dashboard Counts Found For Society" });
            }else{
                return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Dashboard Counts Found For Society" });
            }
        }else{
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: "Please Pass societyId or wingId or Both Field" });
        }
...
//Create Wings--------------MONIL-----------08/06/2021
router.post("/createAdditionalWings",async function(req,res,next){
    try {
        const { societyId , NoOfWing } = req.body;
    ...
    try {
        ...
        if(getMember.length == 1){
            ...
            if(makeAdmin == 1){
                ...
                let memberTokens = await getSingleMemberPlayerId(memberId);
...
//Get All Staff Entry -----------------MONIL---------07/04/2021
router.post("/getStaffEntry_v2",async function(req,res,next){
    try {
        const { societyId , fromDate , toDate } = req.body;
...
//Get All Staff Entry -----------------MONIL---------03/06/2021
router.post("/getStaffEntry_v3",async function(req,res,next){
    try {
        const { societyId , fromDate , toDate , wingId , flatId } = req.body;
        ...
        if(fromDate != undefined && fromDate != null && fromDate != "" && toDate != undefined && toDate != null && toDate != ""){
            //Between Two Dates
            let dateList = generateDateList(fromDate,toDate);
            let getStaffEntryIs = await staffEntrySchema.aggregate([
                {
                    $match: {
                        $and: [
                            { societyId: mongoose.Types.ObjectId(societyId) },
                    ...
                    $match: {
                        $expr: {
                            $or: [
                                {
                                    $and: [
                                        // { $ne: ["$StaffData.staffCategory","Maid"] },
                                        { "StaffData.staffCategory": { $ne: "Maid" } },
                                        {
                                            "StaffData.workingLocation": {
                                                $elemMatch: {
                                                    wingId: mongoose.Types.ObjectId(wingId),
            ...
            getStaffEntryIs.forEach(entry=>{
                let staffIs = entry.StaffData;

                if(staffIs.length == 1){
                    if(entry.StaffData[0].staffCategory != "Maid"){
                        // console.log(entry);
                        let workingAreas = entry.StaffData[0].workingLocation;
                        if(workingAreas.length > 0){
                            workingAreas.forEach(area=>{
                                if(area.wingId == String(wingId) && area.flatId == String(flatId)){
                                    others.push(entry)
                                }
                            });
                        }
            ...
            // return res.send(getStaffEntryIs)
...
router.post("/shareMemberSocietyDetails",async function(req,res,next){
    try {
        ...
        return res.send({ Data: Message.toString() });
    } catch (error) {
        res.status(200).json({ IsSuccess: false , Message: error.message });
    }
});
...
router.post("/shareMyJiniApp",async function(req,res,next){
    try {
        ...
        const myjiniSchema = require('../models/myjiniModel');

        let getData = await myjiniSchema.aggregate([
            {
                $match: {}
            }
        ]);

        let webLink = getData[0].weblink;
        let appLink = getData[0].appLink;
...
//Add Cab-Driver / Delivery-Boy Categories----------------MONIL-------------31/05/2021
// type- 0 =======> For Cab Driver
// type- 1 =======> For Delivery Boy
router.post("/addVisitorSubCategories",async function(req,res,next){
    try {
        ...
        if(image != undefined && image != null && image != ""){
            const path = 'uploads/visitor/'+'visitor_sub_category'+Date.now()+'.png'
        ...
        if(type == 0){
            let addCabDriverCategory = await new visitorSubCategorySchema({
                name: name,
                type: 0,
                parentCategory: "Cab Driver",
                image: imagePath != undefined ? imagePath : ""
            });

            if(addCabDriverCategory != null){
                try {
                    addCabDriverCategory.save();
                    return res.status(200).json({ 
                        IsSuccess: true, 
                        Data: [addCabDriverCategory], 
                        Message: `Cab Driver Category ${name} Added` 
                    });
                } catch (error) {
                    return res.status(500).json({ IsSuccess: false , Message: error.message });
                }
                ...
                return res.status(200).json({ 
                    IsSuccess: true, 
                    Data: [], 
                    Message: `Cab Driver Category ${name} Not Added` 
                });
            }
        }else if(type == 1){
            let addDeliveryBoyCategory = await new visitorSubCategorySchema({
                name: name,
                type: 1,
                parentCategory: "Delivery Boy",
                image: imagePath != undefined ? imagePath : ""
            });

            if(addDeliveryBoyCategory != null){
                try {
                    await addDeliveryBoyCategory.save();
                    return res.status(200).json({ 
                        IsSuccess: true, 
                        Data: [addDeliveryBoyCategory], 
                        Message: `Delivery Boy Category ${name} Added` 
                    });
                } catch (error) {
                    return res.status(500).json({ IsSuccess: false , Message: error.message });
                }
                ...
                return res.status(200).json({ 
                    IsSuccess: true, 
                    Data: [], 
                    Message: `Delivery Boy Category ${name} not Added` 
                });
            }
        }else{
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: `Please Pass 0 for cabdriver and 1 for delivery boy` });
        }
    } catch (error) {
        res.status(500).json({ IsSuccess: false , Message: error.message });
    }
...
//Get Visitor Sub Category--------------------MONIL-------------------31/05/2021
router.post("/getVisitorSubcategory",async function(req,res,next){
    try {
        const { type } = req.body;
        ...
        if(type == 0){
            let getCabDriverSubCategory = await visitorSubCategorySchema.aggregate([
                {
                    $match: {
                        type: 0
                    }
            ...
            if(getCabDriverSubCategory.length > 0){
                return res.status(200).json({ 
                    IsSuccess: true, 
                    Count: getCabDriverSubCategory.length, 
                    Data: getCabDriverSubCategory, 
                    Message: "Cab Driver Sub Categories Found" 
                });
            }else{
                return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Cab Driver Sub Categories Found" });
            }
        }else if(type == 1){
            let getDeliveryboyCategories = await visitorSubCategorySchema.aggregate([
                {
                    $match: {
                        type: 1
                    }
            ...
            if(getDeliveryboyCategories.length > 0){
                return res.status(200).json({ 
                    IsSuccess: true, 
                    Count: getDeliveryboyCategories.length,
                    Data: getDeliveryboyCategories, 
                    Message: "Delivery Boy Sub Categories Found" 
                });
            }else{
                return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Delivery Boy Sub Categories Found" });
            }
        }else{
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: "Please Pass 0 for cab driver and 1 for delivery boy" })
        }
    } catch (error) {
        res.status(500).json({ IsSuccess: false , Message: error.message });
    }
...
//Send Text Message--------------------MONIL-----------------06/06/2021
router.post("/sendTextMessage",async function(req,res,next){
    try {
        const { mobileNo } = req.body;
        
        let otp = getOTP();
        
        let Message = `Your OTP for Civil Verify Form is ${otp} Please Don't share this to anyone.HARSHI`;

        sendTextMessage(Message,mobileNo);
        ...
        return res.status(200).json({ IsSuccess: true , Data: [dataIs] , Message: "OTP Send" });
    } catch (error) {
        res.status(500).json({ IsSuccess: false , Message: error.message });
    }
});

//Delete Society-------------------MONIL-----------------07/06/2021
router.post("/deleteSociety",async function(req,res,next){
    try {
        const { societyId } = req.body;
        ...
        if(getSociety.length == 1){
            let getSocietyMembers = await getSocietyMember(societyId);

            // return res.send(getSocietyMembers);
            if(getSocietyMembers.length > 0){
                getSocietyMembers.forEach( async memberId=>{
                    let getMember = await memberSchema.aggregate([
                        {
                            $match: {
                                _id: mongoose.Types.ObjectId(memberId)
                    ...
                    if(getMember.length == 1){
                        let MemberProperty = getMember[0].society;
    
                        if(MemberProperty.length > 1){
                            let removeMemberSociety = await memberSchema.updateOne(
                                {
                                    _id: mongoose.Types.ObjectId(memberId)
                                },
                                { 
            ...
            let deleteWings = await wingSchema.deleteMany({ societyId: mongoose.Types.ObjectId(societyId) });
            let deleteFlats = await flatSchema.deleteMany({ societyId: mongoose.Types.ObjectId(societyId) });

            let deleteSociety = await societySchema.findByIdAndDelete(societyId);

            return res.status(200).json({ IsSuccess: true, Data: 1 , Message: "Society Deleted with its Member , Wings & Flats" });
        }else{
            return res.status(200).json({ IsSuccess: true, Data: 0 , Message: "No Society Found" });
        }
    } catch (error) {
        res.status(500).json({ IsSuccess: false , Message: error.message });
    }
...
//Test for janam
router.post("/getSocietyWingDetails",async function(req,res,next){
    try {
        const { societyId } = req.body;

        // let getWings = await wingSchema.find({ societyId: societyId })
        //                                 .populate({
        //                                     path: "societyId",
        //                                     select: "Name ContactMobile"
        //                                 });
...
async function sendTextMessage(message,mobileNo){
    let msgportal = "http://45.114.245.84:6005/api/v2/SendSMS?ApiKey=DiuClA4E3dCNdrj4eJcfC5FS8u3pU1mKFJKJiiJ4hPQ=&ClientId=58b179c1-d1cf-4f99-bfdf-4aa22ab12f21&SenderId=HAARSI&Message=" + message + "&MobileNumbers=" + mobileNo + "&Is_Unicode=false&Is_Flash=false";
    try {
...
//Get multiple member PlayerIds(Both mute and unmute) 
async function getMemberPlayerId_mute_unmute(memberIdList,senderId){
    // console.log(memberIdList);
    let PlayerId = [];
    for(let i=0;i<memberIdList.length;i++){
