...
const skipSocietySchema = require('../models/skipSociety');

router.post("/add",async function(req,res,next){
    try {
        const { societyCode } = req.body;

        let getSocietySkip = await skipSocietySchema.aggregate([
            {
                $match: {}
            }
        ]);

        if(getSocietySkip.length == 1){
            let update = {
                $push: {
            ...
            let updateIs = await skipSocietySchema.findByIdAndUpdate(getSocietySkip[0]._id,update);
        }
        
        // let addSociety = await new skipSocietySchema({
        //     societyCode: skipSociety
        // });
...
router.post("/getSocietyList",async function(req,res,next){
    try {
        ...
        // let skipSociety = ["SOC-071998","SOC-560080","SOC-665862","SOC-995460","SOC-085862","SOC-361127","SOC-512904","SOC-504134","SOC-798616"];
        ...
        let skipSociety = getSocietySkip[0].societyCode;
