...
const advertiseServiceSchema = require('../models/advertiseService');
...
router.post("/getVendorDeals",async function(req,res,next){
    try {
        ...
        if(getDeals.length > 0){
            ...
            return res.status(200).json({ IsSuccess: true , Count: 0 , Data: [] , Message: "Deal Found" });
        }else{
...
//Add Advertise Services-----------------MONIL--------------27/05/2021
router.post("/addAdvertiseServices",async function(req,res,next){
    try {
        const { advertiseId , Title , Body } = req.body;
        ...
        if(getAdvertise.length == 0){
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Advertise Found" });
        }

        let addServices = await new advertiseServiceSchema({
            advertiseId: advertiseId,
            Title: Title,
            Body: Body
        });
        ...
        if(addServices != null){
            try {
                ...
                return res.status(200).json({ IsSuccess: true , Data: [addServices] , Message: "Service Added" });
            } catch (error) {
                return res.status(500).json({ IsSuccess: false , Message: error.message });        
            }
        }
...
//Get Advertise Services--------------MONIL------------27/05/2021
router.post("/getAdvertiseServices",async function(req,res,next){
    try {
        const { advertiseId } = req.body;
        ...
        let getServices = await advertiseServiceSchema.aggregate([
            {
                $match: {
        ...
        if(getServices.length > 0){
            return res.status(200).json({ 
                ...
                Data: getServices, 
                Message: "Advertise Services Found" 
            });
        }else{
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Advertise Services Found" });
        }
    } catch (error) {
        res.status(500).json({ IsSuccess: false , Message: error.message });
    }
