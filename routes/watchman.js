        ...
        if(societyCode != undefined){
            ...
            if(societyIs.length == 1){
                ...
                if(watchmanRecord != null){
                    try {
                        await watchmanRecord.save();    
                    } catch (error) {
                        return res.status(500).json({ IsSuccss: false , Message: error.message });
                    }
...
router.post("/updateWatchmanPlayerId",async function(req,res,next){
    try {
        const { watchmanId , mobileNo1 , playerId , DeviceType } = req.body;
        let authToken = req.headers['authorization'];
...
router.post("/addVisitorEntry", visitorImg.fields([
    ...
    try {
        ...
        if(entryNo == undefined || entryNo == null || entryNo == "" || ( isGuest == false && isStaff == false && isVendor == false && isWatchman == false) ){
            ...
            console.log("================NEW VISITOR DATA FORM ENTRY==========================");
            ...
            let addVisitor = await new guestEntrySchema({
                ...
                isMask: isMask,
                status: isVerified == String(true) ? "1" : "0",
...
//Reject Visitor Call------------MONIL----------------11/06/2021
router.post("/rejectCallByWatchmanForUnknownVisitorEntry", async function(req,res,next){
    try {   

        const { watchmanId , callingId } = req.body;
        ...
        let getCall = await unknownVisitorSchema.aggregate([
            {
                $match: {
        ...
        if(getCall.length == 0){
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Call Data Found For Visitor Entry" });
        }
        ...
        let receiverMember = getCall[0].responseMemberId;

        let updateMemberCall = await memberSchema.findByIdAndUpdate(receiverMember,update);
        let updateWatchmanCall = await watchmanSchema.findByIdAndUpdate(watchmanId,update);

        let getReceiverTokens = await getSingleMemberPlayerId(receiverMember);

        if(getReceiverTokens.length > 0){   
            let titleIs = `Incoming Call Is Rejected By Watchman`;
            ...
            getReceiverTokens.forEach(tokenIs=>{
                if(tokenIs.DeviceType == "IOS"){                                    
                    let message = { 
                        app_id: process.env.APP_ID_IOS,
                        contents: {
                    ...
                    sendOneSignalNotification(message,true,false,tokenIs.memberId,watchmanId,"RejectVideoCallingBySender","IOS");
                }else{
                    let message = { 
                        app_id: process.env.APP_ID,
                        contents: {
                    ...
                    sendOneSignalNotification(message,true,false,tokenIs.memberId,watchmanId,"RejectVideoCallingBySender","Android");
                }
            });
            ...
            return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Receiver Token Found" });
        }
    } catch (error) {
        res.status(500).json({ IsSuccss: false , Message: error.message });
    }
    ...
    try {
        ...
        if(fromDate != undefined && fromDate != null && fromDate != "" && toDate != undefined && toDate != null && toDate != ""){
            ...
            if(getAllVisitors.length > 0){
                return res.status(200).json({ 
                    IsSuccess: true, 
                    Count: getAllVisitors.length, 
                    Data: getAllVisitors, 
                    Message: `Visitors Entry between dates ${fromDate} to ${toDate} Found` 
                });
            }else{
                return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Visitors Entry Found" });
            }
            ...
            if(getAllVisitors.length > 0){
                return res.status(200).json({ 
                    ...
                    Count: getAllVisitors.length, 
                    Data: getAllVisitors, 
                    Message: `Visitors Entry Found for Date ${toDate}` 
                });
            }else{
                return res.status(200).json({ IsSuccess: true , Data: [] , Message: "No Visitors Entry Found" });
            }
...
router.post("/logout",async function(req,res,next){
    try {
        const { watchmanId , playerId } = req.body;
        ...
        let getWatchmanToken = await watchmanTokenSchema.aggregate([
            {
                $match: {
                    $and: [
                        { watchmanId: mongoose.Types.ObjectId(watchmanId) },
                        { playerId: playerId }
                    ]
                }
            }
        ...
        if(getWatchmanToken.length == 1){
            let tokenId = getWatchmanToken[0]._id;
            let deleteWatchmanToken = await watchmanTokenSchema.findByIdAndDelete(tokenId);
        }

        return res.status(200).json({ IsSuccess: true , Data: 1 , Message: "Watchman LoggedOut & PlayerId Deleted" });
    } catch (error) {
        res.status(500).json({ IsSuccess: false , Message: error.message });
    }
});
